...
{
        uLocalRcvBuffer[1] = uLocalXmtBuffer[0] = 0;
	...
	uLocalRcvBuffer[7] = uLocalXmtBuffer[7] = 0;
        ...
        // Convert to MCHP
	mTOOLS_CO2MCHP(0x04 + 0xC0000180L);

	// Store the COB
	mRPDOSetCOB(1, mTOOLS_GetCOBID());

	// Convert to MCHP
	mTOOLS_CO2MCHP(0x04 + 0xC0000200L);
...
void Master_ProcessEvents(void)
{
        ...
        if(ButtonPressed()) {
                NMTMaster_Transmit(0x00, nmtstate);
                if(nmtstate == 0x01)
                        nmtstate = 0x02;
                else
                        nmtstate = 0x01;
